...
const (
	DefaultSaleCardSlotSpecial55 = "special55"
	DefaultSaleCardSlotMonth     = "month"
	DefaultSaleCardTokenGroup    = "mix"
)

func DefaultSaleCardSlotDefinitions() []ScheduleSlotDefinition {
	return []ScheduleSlotDefinition{
		{Group: DefaultSaleCardSlotSpecial55, Label: "55 次混合特惠卡", Time: "09:00"},
		{Group: DefaultSaleCardSlotMonth, Label: "月次卡", Time: "09:30"},
	}
}

func DefaultSaleCardPlanTemplates() map[string]SaleCardPlan {
	return map[string]SaleCardPlan{
		"fufu-mix-special-55": {
			ID:            "fufu-mix-special-55",
			Name:          "FuFu 55次混合特惠卡",
			Quota:         55,
			Group:         DefaultSaleCardTokenGroup,
			Slot:          DefaultSaleCardSlotSpecial55,
			IntervalUnit:  3,
			ItemID:        29,
			SKUID:         66,
...
func SaleCardPlanSlot(templates map[string]SaleCardPlan, planID string) string {
	template, ok := templates[strings.TrimSpace(planID)]
	if !ok {
		return ""
	}
...
func DefaultSaleCardSchedule(catalog ScheduleCatalog) ScheduleConfig {
	...
	for _, def := range catalog.Slots {
		slots = append(slots, ScheduleSlot{
			...
			Time:    def.Time,
			Enabled: false,
			Jobs:    DefaultScheduleJobs(def.Group, catalog.Plans),
		})
	}

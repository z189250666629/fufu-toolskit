...
func newApp(cfg Config, db *sql.DB) *App {
	...
	return &App{
		...
		sessions: make(map[string]SessionInfo), authFailures: make(map[string]authFailureRecord), authFailureDelay: authFailureDelay, activeSearches: make(map[string]struct{}), mergeJobTimeout: mergeJobExecutionTimeout,

...
| `apps/fufu-tool-site/ui/src/admin/*` | `AdminPage.tsx` 缩成认证、加载、保存和布局；站点/导航、MCY、售卡、活动配置、通用 UI 组件各自成文件。 | 这是浏览器 UI 和后台 API 编排，依赖 React、HeroUI、fetch 和页面状态，不进 core。 |
...
2. 对 `apps/fufu-act` 剩余 handler 做同样的入口/编排/store/response 拆薄，但已有 `spin`、`scratch`、`shop`、`sale-card`、`login`、`credit` 的主边界，不需要再把外部依赖硬塞进 core。
